...
export function PhaseKpi({ value, label, color }: KpiProps) {
  return (
    <div className="bg-[#f8fafc] rounded-2xl p-4 text-center">
      <div className="text-[22px] font-bold text-[#1e293b]" style={color ? { color } : {}}>
        {value}
      </div>
...
export function PhaseProgressBar({ pct, color, label, total, found }: ProgressBarProps) {
  return (
    <div className="bg-[#f8fafc] rounded-2xl p-4">
      <p className="text-xs text-[#64748b] mb-2">Contribución al total conciliado</p>
      ...
      <p className="text-xs text-[#64748b] mt-2">
        {label} encontró <strong className="text-[#1e293b]">{found.toLocaleString()}</strong> de{" "}
        <strong className="text-[#1e293b]">{total.toLocaleString()}</strong> conciliados totales
      </p>
    </div>
  );
}
...
export function PhaseEmpty({ phase }: EmptyProps) {
  ...
  return (
    <div
      ...
      style={{ boxShadow: "0 4px 20px rgba(0,0,0,0.04)", borderTop: `5px solid ${cfg.color}` }}
    >
      <FileSpreadsheet className="w-14 h-14 mx-auto mb-4" style={{ color: cfg.color, opacity: 0.3 }} />
      <h3 className="text-lg font-bold text-[#1e293b] mb-2">{cfg.label}</h3>
      <p className="text-sm text-[#64748b]">Ejecuta una conciliación para ver resultados de esta fase.</p>
    </div>
  );
}
...
export function MonthHeader({ monthIndex, year, isFallback }: MonthHeaderProps) {
  return (
    <div className="flex items-center gap-2 flex-wrap mb-1">
      <div className="flex items-center gap-2 text-[#64748b] text-sm">
        <Calendar className="w-4 h-4 text-[#667eea]" />
        <span>
          Datos de <strong className="text-[#1e293b]">{getMonthName(monthIndex)} {year}</strong>
        </span>
      </div>
      {isFallback && (
...
export function PhaseHeroCard({ phase, desc, cuenta, children }: PhaseHeroCardProps) {
  const cfg = PHASE_CONFIG[phase];
  return (
    <div
