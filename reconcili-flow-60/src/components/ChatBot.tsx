...
function getResponse(msg: string, ctx: string, mesLabel: string): string {
  ...
  if (
    q.includes("tasa") ||
    q.includes("efectividad") ||
    (q.includes("%") && q.includes("concilia"))
  ) {
  ...
  if (
    q.includes("f5") ||
    q.includes("monto puro") ||
    (q.includes("monto") && q.includes("global"))
  ) {
  ...
  if (
    q.includes("f6") ||
    (q.includes("subset") && q.includes("global")) ||
    q.includes("n positivos") ||
    q.includes("neg. grandes")
  ) {
  ...
  if (
    q.includes("f7b") ||
    q.includes("limpieza final") ||
    q.includes("final cleaning")
  ) {
  ...
  if (
    q.includes("f7") ||
    q.includes("1 positivo") ||
    q.includes("final clean")
  ) {
...
export default function ChatBot() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "¡Hola! Soy el asistente del Conciliador Pro. Puedo ayudarte a entender las fases del motor (F1–F7b), interpretar resultados, navegar la aplicación y descargar reportes. ¿En qué te puedo ayudar?",
    },
  ]);
  ...
  const send = async (text?: string) => {
    ...
    setMessages((prev) => [
      ...prev,
      { id: Date.now() + 1, role: "assistant", content: respuesta },
    ]);
    setLoading(false);
  };
    ...
    <div className="fixed bottom-6 right-6 w-96 h-[520px] bg-card rounded-2xl shadow-2xl flex flex-col overflow-hidden z-50 border border-border">
      ...
      <div className="gradient-primary px-4 py-3 flex justify-between items-center flex-shrink-0">
        ...
        <div className="flex gap-1">
          <button
            ...
            className="p-1.5 text-primary-foreground/70 hover:text-primary-foreground hover:bg-primary-foreground/20 rounded-lg transition-colors"
          >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar">
        {messages.map((msg) => (
          ...
          >
            <div
              className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${msg.role === "assistant" ? "bg-primary/10" : "bg-muted"}`}
            >
              {msg.role === "assistant" ? (
