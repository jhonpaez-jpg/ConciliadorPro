...
export default function ChartSection({
  ...
  const barData = [
    ...
    {
      name: "Conciliados",
      valor: safeConciliados,
      fill: "hsl(var(--success))",
      pct: tasaReal.toFixed(1) + "%",
    },
    {
      name: "Pendientes",
      valor: safePendientes,
      fill: "hsl(var(--warning))",
      pct: (100 - tasaReal).toFixed(1) + "%",
    },
  ];
    ...
    return (
      <div className="bg-card border border-border rounded-xl p-3 shadow-lg text-sm">
        ...
        <p className="text-muted-foreground">
          {(d.valor ?? 0).toLocaleString()} registros
        </p>
    ...
    <div className="bg-card rounded-2xl p-6 shadow-card">
      ...
      <div className="grid grid-cols-4 gap-4 mt-6">
        {[
          ...
          {
            label: "Conciliados",
            value: safeConciliados.toLocaleString(),
            color: "text-success",
          },
          {
            label: "Pendientes",
            value: safePendientes.toLocaleString(),
            color: "text-warning",
          },
          {
            label: "Tasa real",
            value: tasaReal.toFixed(2) + "%",
            color: tasaReal >= 80 ? "text-success" : "text-warning",
          },
