...
export default function MonthlyNav() {
  ...
  const efectividad =
    total > 0 ? ((conciliados / total) * 100).toFixed(0) + "%" : "—";
  ...
  return (
    <header className="monthly-nav px-8 py-4 flex justify-between items-center flex-shrink-0 border-b border-white/10">
      <div className="flex items-center gap-5 bg-sidebar-border/30 px-4 py-2 rounded-full">
        <button
          onClick={prevMonth}
          className="w-9 h-9 rounded-full flex items-center justify-center text-sidebar-muted hover:bg-sidebar-border/50 hover:text-sidebar-fg transition-colors"
        >
