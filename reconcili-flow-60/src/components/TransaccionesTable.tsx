            ...
            <tbody>
              {items.map((item, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-card-foreground max-w-[200px] truncate">
                    {item.descripcion}
                  </td>
                  <td className="px-4 py-3 text-card-foreground">
                    {item.localidad}
                  ...
                  <td
                    className={`px-4 py-3 font-medium ${(item.monto ?? 0) >= 0 ? "text-success" : "text-destructive"}`}
                  >
                    {(item.monto ?? 0) >= 0 ? "+" : ""}
                  ...
                  {isConciliado && (
                    <td className="px-4 py-3">
                      <span
                        className={`px-2 py-0.5 rounded text-xs font-medium ${FASE_COLORS[item.fase ?? ""] ?? "bg-muted text-muted-foreground"}`}
                      >
                        {item.fase ?? "—"}
                      </span>
                    </td>
