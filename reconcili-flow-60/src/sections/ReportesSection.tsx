...
export default function ReportesSection() {
  ...
  const downloadEspecializado = async (
    endpoint: string,
    nombre: string,
    tipo: "xlsx" | "pdf",
  ) => {
  ...
  const reportes = [
    {
      ...
      badge: hasData
        ? `${conciliados.toLocaleString()} conciliados`
        : undefined,
      badgeColor: "bg-success/10 text-success",
      desc: "LOGRADO por fase (F1–F7b), grupos, IDs, combinaciones encontradas",
      onExcel: hasData
        ? () =>
            downloadEspecializado(
              "reporte-conciliacion-completa",
      ...
      onPdf: hasData
        ? () =>
            downloadEspecializado(
              "reporte-conciliacion-completa-pdf",
              "conciliacion_completa",
              "pdf",
            )
        : undefined,
      ...
      <div className="bg-card rounded-2xl p-6 shadow-card">
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          <span className="text-sm text-muted-foreground">
            ...
            {hasData && cuenta && (
              <span className="ml-2">
                — Cuenta {cuenta} — {total.toLocaleString()} registros
              </span>
            )}
          </span>
          {isFallback && (
              ...
              >
                ...
                <div className="flex-1 min-w-0 mx-4">
                  <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                    ...
                    {r.badge && (
                      <span
                        className={`px-2 py-0.5 rounded-full text-[11px] font-medium ${r.badgeColor}`}
                      >
                        {r.badge}
                      </span>
                    )}
        ...
        <div className="bg-card rounded-2xl p-6 shadow-card">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
            {[
              {
                hoja: "RESUMEN",
                desc: "Estadísticas globales y desglose por fase",
              },
