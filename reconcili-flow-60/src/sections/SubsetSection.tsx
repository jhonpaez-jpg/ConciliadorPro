  ...
  if (!cuenta || !stats) {
    return (
      <div className="bg-card rounded-2xl p-12 shadow-card text-center">
        ...
        <h3 className="text-lg font-semibold text-card-foreground mb-2">F6: Monto Puro Subset</h3>
        <p className="text-muted-foreground text-sm">Ejecuta una conciliación para ver resultados de esta fase.</p>
      </div>
    );
  }

  const f6 = stats.fases?.F6 ?? { conciliados: 0, pct_sobre_conciliados: 0, pct_sobre_total: 0 };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-card rounded-2xl p-6 shadow-card space-y-6">
        <div>
          <h3 className="text-lg font-semibold text-card-foreground mb-2">F6: Monto Puro Subset</h3>
          <p className="text-muted-foreground text-sm">Suma de parejas exactas solo por monto, sin localidad ni número de diario — Cuenta: {cuenta}</p>
        </div>
        <div className="grid grid-cols-3 gap-5">
          <StatCard title="Conciliados F6" value={f6.conciliados.toLocaleString()} icon={Cpu} />
          <StatCard title="% sobre conciliados" value={`${f6.pct_sobre_conciliados.toFixed(1)}%`} icon={CheckCircle} />
          <StatCard title="% sobre total" value={`${f6.pct_sobre_total.toFixed(1)}%`} icon={CheckCircle} />
        </div>
        <div className="bg-muted/50 rounded-2xl p-4">
          <p className="text-xs text-muted-foreground mb-2">Contribución de F6 al total conciliado</p>
          <div className="w-full bg-border rounded-full h-3">
            <div className="bg-primary h-3 rounded-full transition-all" style={{ width: `${Math.min(f6.pct_sobre_conciliados, 100)}%` }} />
          </div>
          <p className="text-xs text-muted-foreground mt-2">
            F6 encontró {f6.conciliados.toLocaleString()} de {(stats.total_conciliados ?? 0).toLocaleString()} conciliados totales
          </p>
        </div>
      </div>
    </div>
