...
export default function DashboardSection() {
  ...
  const dataMes: any =
    ultimoDelMes ??
    (result?.mes === currentMonthIndex && result?.anio === currentYear
      ? result
      : null) ??
    history[0] ??
    null;
  ...
  const efectividad =
    tasa > 0
      ? tasa.toFixed(1)
      : totalRegistros > 0
        ? ((conciliados / totalRegistros) * 100).toFixed(1)
        : "0";
      ...
      {!vistaDetalle && (
        <>
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            <PhaseBadge
              phase="F6"
              label="F6: Subset Global"
              icon={<Layers className="w-3.5 h-3.5" />}
              variant="f2"
            />
            <PhaseBadge
              phase="F7"
              label="F7: Final Cleaning"
              icon={<Zap className="w-3.5 h-3.5" />}
              variant="f1"
            />
            <PhaseBadge
              phase="F7b"
              label="F7b: Limpieza Final"
              icon={<Zap className="w-3.5 h-3.5" />}
              variant="f1"
            />
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {hasData && !esDatoDelMes && (
              <span className="px-3 py-1 rounded-full text-xs font-medium bg-warning/10 text-warning">
                ⚠ Mostrando última conciliación disponible — sin datos para{" "}
                {mesLabel}
              </span>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5">
            <StatCard
              ...
              value={hasData ? totalRegistros.toLocaleString() : "—"}
              trend={hasData ? `Cuenta: ${cuenta}` : "Sin datos aún"}
              icon={Database}
            />
            <StatCard
              title="Conciliados"
              value={hasData ? conciliados.toLocaleString() : "—"}
              trend={
                hasData
                  ? `${efectividad}% del total`
            ...
            <StatCard
              title="Pendientes"
              value={hasData ? pendientes.toLocaleString() : "—"}
              trend={hasData ? "Requieren revisión manual" : "—"}
              icon={Hourglass}
            />
            <StatCard
              title="Efectividad"
              value={hasData ? `${efectividad}%` : "—"}
              trend={
                hasData
                  ? `${conciliados.toLocaleString()} de ${totalRegistros.toLocaleString()}`
                  : "—"
              }
              icon={ClockIcon}
            />
            ...
            <div className="flex gap-4 items-center flex-wrap">
              ...
              {hasData && (
                <>
                  <button
                    onClick={() =>
                      setVistaDetalle({ estado: "CONCILIADO", cuenta })
                    }
                  ...
                  <button
                    onClick={() =>
                      setVistaDetalle({ estado: "PENDIENTE", cuenta })
                    }
              ...
              {ultimoDelMes && (
                <span className="text-muted-foreground text-[13px] ml-auto flex items-center gap-1.5">
                  <ClockIcon className="w-3.5 h-3.5" /> Última:{" "}
                  {ultimoDelMes.date}
                </span>
              )}
            </div>
          ...
          {!hasData && history.length === 0 && (
            <div className="bg-card rounded-2xl p-12 shadow-card text-center">
              ...
              <h3 className="text-lg font-semibold text-card-foreground mb-2">
                Sin datos para {mesLabel}
              </h3>
              <p className="text-muted-foreground text-sm mb-6 max-w-md mx-auto">
                Sube un archivo Excel (.xlsx) en la sección "Ejecutar
                Conciliación" para procesar tus transacciones bancarias.
              </p>
              <button
                onClick={() => setActiveSection("ejecutar")}
          ...
          {history.length > 0 && !hasData && historialMes.length === 0 && (
            <div className="bg-card rounded-2xl p-6 shadow-card text-center">
              <p className="text-muted-foreground text-sm">
                No hay conciliaciones registradas para{" "}
                <strong className="text-card-foreground">{mesLabel}</strong>.
              </p>
              <p className="text-muted-foreground text-xs mt-2">
                Usa las flechas ← → para navegar al mes con datos, o ejecuta una
                nueva conciliación.
              </p>
            </div>
          )}
          ...
          {historialMes.length > 0 && (
            <div className="bg-card rounded-2xl p-6 shadow-card">
              <h3 className="text-base font-semibold text-card-foreground mb-4">
                Ejecuciones de {mesLabel}
              </h3>
              <div className="space-y-2">
                {historialMes.slice(0, 5).map((h, i) => (
                  <div
                    key={i}
                    className="flex items-center p-4 bg-muted/50 rounded-2xl hover:bg-muted transition-colors"
                  >
                    <div className="w-[100px] font-medium text-card-foreground text-sm">
                      {h.date}
                    </div>
                    <div className="flex-1 flex gap-6">
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <span className="text-sm font-semibold text-card-foreground">
                          {(h.conciliados ?? 0).toLocaleString()} (
                          {h.efectividad})
                        </span>
                      </div>
                      <div className="flex flex-col gap-1">
