...
export default function ProfundaSection() {
  ...
  useEffect(() => {
    if (!cuenta) return;
    axios
      .get("/api/stats-por-fase/", { params: { cuenta } })
    ...
    return (
      <div className="bg-card rounded-2xl p-12 shadow-card text-center">
        ...
        <h3 className="text-lg font-semibold text-card-foreground mb-2">
          Resumen Fases Avanzadas (F3–F5)
        </h3>
        <p className="text-muted-foreground text-sm">
          Ejecuta una conciliación para ver resultados.
        </p>
      </div>
    );
  }

  const f3 = stats.fases?.F3 ?? {
    conciliados: 0,
    pct_sobre_conciliados: 0,
    pct_sobre_total: 0,
  };
  const f4 = stats.fases?.F4 ?? {
    conciliados: 0,
    pct_sobre_conciliados: 0,
    pct_sobre_total: 0,
  };
          ...
          {phases.map(({ key, label, icon: Icon, data, color }) => (
            <div key={key} className="bg-muted/50 rounded-2xl p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <StatCard
                  title="Conciliados"
                  value={data.conciliados.toLocaleString()}
                  icon={CheckCircle}
                />
                <StatCard
                  title="% total"
                  value={`${data.pct_sobre_total.toFixed(1)}%`}
                />
              </div>
              <div>
                <div className="w-full bg-border rounded-full h-2">
                  <div
                    ...
                    style={{
                      width: `${Math.min(data.pct_sobre_conciliados, 100)}%`,
                    }}
                  />
                </div>
                <p className="text-[11px] text-muted-foreground mt-1">
                  {data.pct_sobre_conciliados.toFixed(1)}% de conciliados
                </p>
              </div>
            </div>
          ))}
