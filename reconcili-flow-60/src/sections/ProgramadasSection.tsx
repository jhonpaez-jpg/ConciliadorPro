...
import {
  CalendarClock, CheckCircle, XCircle, Loader2, Clock,
  Trash2, ChevronDown, ChevronUp, Timer, RefreshCw,
...
const ESTADO_CFG = {
  PENDIENTE:  { color: "#667eea", bg: "rgba(102,126,234,0.07)", label: "Pendiente",  Icon: Clock },
  EJECUTANDO: { color: "#f59e0b", bg: "rgba(245,158,11,0.07)",  label: "Ejecutando", Icon: Loader2 },
  COMPLETADO: { color: "#10b981", bg: "rgba(16,185,129,0.07)",  label: "Completado", Icon: CheckCircle },
  ERROR:      { color: "#ef4444", bg: "rgba(239,68,68,0.07)",   label: "Error",      Icon: XCircle },
  CANCELADO:  { color: "#94a3b8", bg: "rgba(148,163,184,0.07)", label: "Cancelado",  Icon: XCircle },
} as const;
...
function ProgramadaRow({
  ...
  const cfg = ESTADO_CFG[item.estado as keyof typeof ESTADO_CFG] ?? ESTADO_CFG.PENDIENTE;
  const { Icon } = cfg;
  const pct = item.estado === "EJECUTANDO" && estadoBackend
    ? getProgressPct(estadoBackend.mensaje ?? "")
    : item.estado === "COMPLETADO" ? 100 : 0;
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-3 p-5">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            <span className="text-sm font-bold text-[#1e293b] truncate">{item.archivo}</span>
          ...
          <div className="flex items-center gap-4 text-xs text-[#64748b] flex-wrap">
            <span>
              ...
              {item.fecha} · <strong className="text-[#1e293b]">{item.hora}</strong>
            </span>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          {(item.estado === "EJECUTANDO" || item.estado === "COMPLETADO" || item.estado === "ERROR") && (
            <button
        ...
        <div className="px-5 pb-3">
          <div className="flex items-center justify-between mb-1">
            <span className="text-xs font-medium" style={{ color: cfg.color }}>
              {item.estado === "COMPLETADO" ? "Completado" : estadoBackend?.mensaje || "Procesando..."}
            </span>
            <span className="text-xs font-mono font-bold" style={{ color: cfg.color }}>{pct}%</span>
          </div>
      ...
      {/* Detalle expandido */}
      {expanded && estadoBackend && (item.estado === "EJECUTANDO" || item.estado === "COMPLETADO") && (
        <div className="px-5 pb-5 pt-1 border-t border-white/40 space-y-4">
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: "Conciliados",  value: (estadoBackend.conciliados ?? 0).toLocaleString(), color: "#10b981" },
              { label: "Pendientes",   value: (estadoBackend.pendientes  ?? 0).toLocaleString(), color: "#f59e0b" },
              { label: "Efectividad",  value: `${(estadoBackend.tasa ?? 0).toFixed(1)}%`,        color: "#667eea" },
          ...
          {item.estado === "COMPLETADO" && (
            <p className="text-xs text-[#10b981] font-semibold flex items-center gap-1.5">
              <CheckCircle className="w-3.5 h-3.5" /> Conciliación completada exitosamente
            </p>
          )}
        </div>
      )}
...
export default function ProgramadasSection() {
  ...
  const ORDER = { EJECUTANDO: 0, PENDIENTE: 1, COMPLETADO: 2, ERROR: 3, CANCELADO: 4 };
  const sorted = [...scheduled].sort((a, b) =>
    (ORDER[a.estado as keyof typeof ORDER] ?? 5) - (ORDER[b.estado as keyof typeof ORDER] ?? 5)
  );
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-white rounded-[20px] p-6" style={{ boxShadow: "0 4px 20px rgba(0,0,0,0.04)" }}>
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-[#1e293b]">Ejecuciones Programadas</h2>
              <p className="text-xs text-[#64748b]">Monitor en tiempo real de conciliaciones automáticas</p>
            </div>
          </div>
          <button
        ...
        <div className="grid grid-cols-4 gap-3">
          {[
            { label: "Ejecutando",  value: counts.ejecutando,  color: "#f59e0b", bg: "rgba(245,158,11,0.08)" },
            { label: "Pendientes",  value: counts.pendientes,  color: "#667eea", bg: "rgba(102,126,234,0.08)" },
            { label: "Completadas", value: counts.completadas, color: "#10b981", bg: "rgba(16,185,129,0.08)" },
            { label: "Errores",     value: counts.errores,     color: "#ef4444", bg: "rgba(239,68,68,0.08)" },
          ].map(({ label, value, color, bg }) => (
            <div key={label} className="rounded-2xl p-4 text-center" style={{ background: bg }}>
              <div className="text-2xl font-bold" style={{ color }}>{value}</div>
              <div className="text-[11px] text-[#64748b] uppercase tracking-wide mt-0.5">{label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Lista vacía */}
      {sorted.length === 0 && (
        <div className="bg-white rounded-[20px] p-12 text-center" style={{ boxShadow: "0 4px 20px rgba(0,0,0,0.04)" }}>
          <FileSpreadsheet className="w-14 h-14 mx-auto mb-4 text-[#cbd5e1]" />
          <h3 className="text-base font-bold text-[#1e293b] mb-2">Sin ejecuciones programadas</h3>
          <p className="text-sm text-[#64748b]">
            Ve a <strong>Ejecutar Conciliación</strong> y usa el botón{" "}
            <strong>"Programar para nocturno"</strong> para agendar una ejecución automática.
          </p>
        </div>
      )}
        ...
        <div className="space-y-3">
          {sorted.map(item => (
            <ProgramadaRow
              key={item.id}
              item={item}
              estadoBackend={item.estado === "EJECUTANDO" || item.estado === "COMPLETADO" ? estadoBackend : null}
