    ...
    <div className="space-y-4">
      ...
      <PhaseHeroCard
        ...
        desc={
          <>
            Captura grupos por{" "}
            <code className="bg-[#ede9fe] text-[#5b21b6] px-1.5 py-0.5 rounded text-[12px] font-mono">
              n_diario + localidad
            </code>{" "}
            cuya suma no es exactamente cero sino dentro de{" "}
            <strong>±5 centavos</strong>. Cubre diferencias de redondeo entre
            sistemas contables. Históricamente vacía en los datos reales (buena
            señal de calidad).
          </>
        }
      >
        <div className="grid grid-cols-4 gap-4">
          <PhaseKpi
            value={f3.conciliados.toLocaleString()}
            label="Registros conciliados"
            color="#8b5cf6"
          />
          <PhaseKpi
            value={`${f3.pct_sobre_conciliados.toFixed(1)}%`}
            label="% sobre conciliados"
          />
          <PhaseKpi
            value={`${f3.pct_sobre_total.toFixed(1)}%`}
            label="% sobre total"
          />
