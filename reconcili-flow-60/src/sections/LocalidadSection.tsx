      ...
      <PhaseHeroCard
        ...
        desc={
          <>
            ...
            <code className="bg-[#dbeafe] text-[#1d4ed8] px-1.5 py-0.5 rounded text-[12px] font-mono">
              n_diario
            </code>
            . Cruza registros con{" "}
            <strong>mismo monto exacto y misma localidad</strong>,
            independientemente del número de diario. Captura cruces SIF82↔TES82
            que comparten localidad pero no n_diario.
          </>
        }
      >
        <div className="grid grid-cols-4 gap-4">
          <PhaseKpi
            value={f4.conciliados.toLocaleString()}
            label="Registros conciliados"
            color="#3b82f6"
          />
          <PhaseKpi
            value={`${f4.pct_sobre_conciliados.toFixed(1)}%`}
            label="% sobre conciliados"
          />
          <PhaseKpi
            value={`${f4.pct_sobre_total.toFixed(1)}%`}
            label="% sobre total"
          />
