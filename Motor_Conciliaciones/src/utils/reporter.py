...
class ReconciliationReporter:
    ...
    def generar_excel_desde_db(self, db, cuenta: str, id_lote: int):
        ...
        nombre_archivo = os.path.join(self.ruta_salida, f"resultado_conciliacion_{timestamp}.xlsx")
        ...
        fase_config = {
            ...
            "F6": {
                "color": "#7b2d00",
                "nombre": "F6 - Subset Sum (N positivos → 1 negativo)",
                "fmt": None,
            },
            "F7": {
                "color": "#5a2d00",
                "nombre": "F7 - Subset Sum (1 positivo → N negativos)",
                "fmt": None,
            },
            "F7b": {
                "color": "#3d1f00",
                "nombre": "F7b - Final Cleaning (positivos restantes)",
                "fmt": None,
            },
        }
        ...
        def _escribir_hoja_fase(ws, fmt_hdr, fase_label, nombre_fase):
            ...
            fmt_desc = workbook.add_format(
                {"italic": True, "font_color": "#555555", "font_size": 9}
            )
            ...
            if fila > 2:
                ...
                ws.write(
                    2,
                    0,
                    f"Sin registros conciliados por {fase_label} en este lote.",
        ...
        print(
            f"📊 Reporte generado: {nombre_archivo} "
            f"({total_conci:,} conciliados | {total_pend:,} pendientes)"
        )
        # Devolver solo el nombre del archivo (sin ruta) para que sea portable
        # entre entornos local y desplegado
