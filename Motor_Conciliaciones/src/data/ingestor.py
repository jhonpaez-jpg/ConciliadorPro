...
class DataIngestor:
    ...
    def _leer_hoja_correcta(self) -> "pl.DataFrame":
        ...
        nombres_comunes = ["Hoja1", "Hoja 1", "Sheet1", "Sheet 1", "Data", "Datos", "data"]
        ...
        for candidato in candidatos:
            try:
                kwargs = (
                    {"sheet_name": candidato}
                    if isinstance(candidato, str)
                    else {"sheet_id": candidato + 1}
                )  # polars usa 1-based
                df_test = pl.read_excel(self.ruta_archivo, engine="calamine", **kwargs)
    ...
    def procesar_excel(self) -> pl.DataFrame:
        ...
        col_ndiario = next(
            (
                c
                for c in ["N.º diario", "N.º Diario", "Nº diario", "N° diario", "Diario"]
                if c in df.columns
            ),
            None,
        )
        col_localidad = next(
            (c for c in ["Localidad", "localidad", "LOCALIDAD"] if c in df.columns), None
        )
        ...
        df_sel = df.select(
            [
                ...
                (pl.col("Importe").fill_null(0) * 100)
                .round(0)
                .cast(pl.Int64)
                .alias("monto_centavos"),
        ...
        # Validación: solo se permite un período por archivo
        if len(periodos) > 1:
            raise ValueError(
                f"El archivo contiene datos de {len(periodos)} períodos diferentes: "
                f"{', '.join(periodos)}. "
                f"Sube un archivo con datos de un solo período contable (ej: solo 2026/001)."
            )

        df_limpio = (
            df_sel.with_columns(pl.col("n_diario_raw").cast(pl.Int64).alias("n_diario")).drop(
                "n_diario_raw"
            )
