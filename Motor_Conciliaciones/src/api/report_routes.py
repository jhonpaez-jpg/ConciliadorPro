...
def _pdf_base_styles():
    ...
    S = {
        "titulo":   ParagraphStyle("t",  fontName="Helvetica-Bold", fontSize=20, textColor=C["blanco"], alignment=TA_CENTER),
        "sub":      ParagraphStyle("s",  fontName="Helvetica",      fontSize=10, textColor=colors.HexColor("#93c5fd"), alignment=TA_CENTER),
        "seccion":  ParagraphStyle("sc", fontName="Helvetica-Bold", fontSize=11, textColor=C["azul"],   spaceBefore=12, spaceAfter=5),
        "normal":   ParagraphStyle("n",  fontName="Helvetica",      fontSize=8,  textColor=C["negro"],  leading=11),
        "small":    ParagraphStyle("sm", fontName="Helvetica",      fontSize=7,  textColor=C["muted"],  leading=10),
        "pie":      ParagraphStyle("p",  fontName="Helvetica",      fontSize=7,  textColor=C["muted"],  alignment=TA_CENTER),
        "bold":     ParagraphStyle("b",  fontName="Helvetica-Bold", fontSize=8,  textColor=C["negro"]),
        "right":    ParagraphStyle("r",  fontName="Helvetica",      fontSize=8,  textColor=C["negro"],  alignment=TA_RIGHT),
        "center":   ParagraphStyle("c",  fontName="Helvetica",      fontSize=8,  textColor=C["negro"],  alignment=TA_CENTER),
        "verde_v":  ParagraphStyle("vv", fontName="Helvetica-Bold", fontSize=8,  textColor=C["verde"]),
...
def _pie_pdf(elems, page_w, filename, C, S):
    ...
    elems.append(Paragraph(
        f"Conciliador Pro  ·  {dt.now().strftime('%d/%m/%Y %H:%M')}  ·  {filename}",
        S["pie"]
    ))
...
async def reporte_conciliacion_completa(id_lote: int = Query(...), cuenta: str = Query(...)):
    ...
    fmt_hdr   = wb.add_format({"bold":True,"bg_color":"#1e3a5f","font_color":"white","border":1,"align":"center"})
    ...
    ws.set_column("A:A", 38); ws.set_column("B:B", 18)
    ws.write(0,0,"Concepto",fmt_hdr); ws.write(0,1,"Valor",fmt_hdr)
    filas = [
    ...
    for i,(k,v) in enumerate(filas,1):
        ws.write(i,0,k,fmt_bold); ws.write(i,1,v)

    # Desglose por fase en RESUMEN
    ws.write(len(filas)+2,0,"Fase",fmt_hdr); ws.write(len(filas)+2,1,"Conciliados",fmt_hdr)
    cur.execute("""
    ...
    for i,(fase,n) in enumerate(cur.fetchall(), len(filas)+3):
        ws.write(i,0,fase); ws.write(i,1,n)
    ...
    for fase_key in ["F1","F2","F3","F4","F5","F6","F7","F7b"]:
        ...
        ws_f.merge_range(0,0,0,6, desc, wb.add_format({"italic":True,"font_color":"#555555","font_size":9}))
        hdrs = ["Grupo_ID","ID_Tx","Tipo","Período","Descripción","Localidad","Monto (COP)"]
        for c_,h in enumerate(hdrs): ws_f.write(1,c_,h,fmt_hdr)
        ws_f.set_column("E:E",35); ws_f.set_column("G:G",14)
        for r_,(gid,tid,tipo,per,desc_,loc,monto) in enumerate(rows,2):
            fmt_bg = fmt_alt if r_%2==0 else None
            ws_f.write(r_,0,gid,fmt_bg); ws_f.write(r_,1,tid,fmt_bg)
            ws_f.write(r_,2,tipo,fmt_bg); ws_f.write(r_,3,per,fmt_bg)
            ws_f.write(r_,4,desc_,fmt_bg); ws_f.write(r_,5,loc,fmt_bg)
            ws_f.write(r_,6,monto, fmt_neg if (monto or 0)<0 else fmt_pos)
        if rows: ws_f.autofilter(1,0,len(rows)+1,6)

    # ── Hoja LOGRADO (todos + fase) ───────────────────────────────
    ws_log = wb.add_worksheet("LOGRADO")
    hdrs_log = ["Grupo_ID","ID_Tx","Tipo","Período","Descripción","Localidad","Monto (COP)","Fase"]
    for c_,h in enumerate(hdrs_log): ws_log.write(0,c_,h,fmt_hdr)
    ws_log.set_column("E:E",35); ws_log.set_column("H:H",6)
    cur.execute("""
        SELECT ABS(t.id_conciliacion), t.id_transaccion, t.tipo, t.periodo,
               t.descripcion, t.localidad, ROUND(CAST(t.monto_centavos AS REAL)/100,2),
    ...
    for r_,row in enumerate(cur.fetchall(),1):
        fmt_bg = fmt_alt if r_%2==0 else None
        for c_,v in enumerate(row): ws_p.write(r_,c_,v,fmt_bg)
...
async def reporte_metricas_fases(id_lote: int = Query(...), cuenta: str = Query(...)):
    ...
    ws.set_column("A:A",12); ws.set_column("B:B",42); ws.set_column("C:H",14)
    hdrs = ["Fase","Descripción","Grupos","Transacciones","% del Total","% Conciliados","Neg. cubiertos","Pos. usados"]
    for c_,h in enumerate(hdrs): ws.write(0,c_,h,fmt_hdr)
    ...
    for r_,row in enumerate(cur.fetchall(),1):
        ...
        ws.write(r_,2,grupos,fmt_bg); ws.write(r_,3,txs,fmt_bg)
        ws.write(r_,4,pct_total,fmt_bg); ws.write(r_,5,pct_conci,fmt_bg)
        ws.write(r_,6,neg,fmt_bg); ws.write(r_,7,pos,fmt_bg)

    # ── Hoja GRUPOS por fase ──────────────────────────────────────
    ws2 = wb.add_worksheet("GRUPOS_POR_FASE")
    ws2.set_column("A:A",8); ws2.set_column("B:B",12); ws2.set_column("C:C",14)
    hdrs2 = ["Fase","Grupo_ID","# Transacciones","Monto Neto (COP)","Suma Positivos","Suma Negativos"]
    for c_,h in enumerate(hdrs2): ws2.write(0,c_,h,fmt_hdr)
    ...
    for r_,row in enumerate(cur.fetchall(),1):
        fmt_bg = fmt_alt if r_%2==0 else None
        for c_,v in enumerate(row): ws2.write(r_,c_,v,fmt_bg)
    if r_ > 0: ws2.autofilter(0,0,r_,5)

    con.close(); wb.close(); buf.seek(0)
...
async def reporte_pendientes(id_lote: int = Query(...), cuenta: str = Query(...)):
    ...
    buf = io.BytesIO()
    wb  = xlsxwriter.Workbook(buf, {"in_memory": True})
    fmt_hdr  = wb.add_format({"bold":True,"bg_color":"#7c2d12","font_color":"white","border":1,"align":"center"})
    ...
    hdrs = ["ID_Tx","Tipo","Período","Descripción","Localidad","Monto (COP)","Contraparte posible"]
    for c_,h in enumerate(hdrs): ws2.write(0,c_,h,fmt_hdr)
    ws2.set_column("D:D",35); ws2.set_column("G:G",20)
    ...
    for r_,row in enumerate(neg_rows,1):
        ...
        fmt_bg = fmt_alt if r_%2==0 else None
        ws2.write(r_,0,tid,fmt_bg); ws2.write(r_,1,tipo,fmt_bg)
        ws2.write(r_,2,per,fmt_bg); ws2.write(r_,3,desc,fmt_bg)
        ws2.write(r_,4,loc,fmt_bg); ws2.write(r_,5,monto,fmt_neg)
        ws2.write(r_,6,tiene_contra,fmt_bg)
    if neg_rows: ws2.autofilter(0,0,len(neg_rows),6)
    ...
    for c_,h in enumerate(hdrs3): ws3.write(0,c_,h,fmt_hdr)
    ...
    for r_,row in enumerate(cur.fetchall(),1):
        fmt_bg = fmt_alt if r_%2==0 else None
        tid,tipo,per,desc,loc,monto = row
        ws3.write(r_,0,tid,fmt_bg); ws3.write(r_,1,tipo,fmt_bg)
        ws3.write(r_,2,per,fmt_bg); ws3.write(r_,3,desc,fmt_bg)
        ws3.write(r_,4,loc,fmt_bg); ws3.write(r_,5,monto,fmt_pos)
    ...
    for c_,h in enumerate(hdrs4): ws4.write(0,c_,h,fmt_hdr)
    ws4.set_column("D:D",35); ws4.set_column("G:G",30)
    cur.execute("""
        SELECT t.id_transaccion, t.tipo, t.periodo, t.descripcion, t.localidad,
    ...
    for row in cur.fetchall():
        ...
        if abs(mc) not in pos_montos:
            ...
            ws4.write(irr+1,0,tid,fmt_bg); ws4.write(irr+1,1,tipo,fmt_bg)
            ws4.write(irr+1,2,per,fmt_bg); ws4.write(irr+1,3,desc,fmt_bg)
            ws4.write(irr+1,4,loc,fmt_bg); ws4.write(irr+1,5,monto,fmt_neg)
...
async def reporte_evolucion_pdf(id_lote: int = Query(...), cuenta: str = Query(...)):
    ...
    versiones = [
        ...
        ["v1 — Original",    "~71%",  "F1, F2, F3, F4",          "Motor base con coincidencias exactas y subset sum local"],
        ["v2 — Bug fixes",   "~76%",  "F1, F2, F3, F4, F5",      "Corrección de 5 bugs críticos. F5 (monto puro) añadida"],
        ["v3 — F7 integrada","~86%",  "F1–F5, F6g, F7, F7b",     "F7 integrada al pipeline. F5 movida al final. F7b añadida"],
        ["v4 — Optimizada",  "~99%+", "F1–F5, F6, F7, F7b",      "F7b: O(neg×pos×DP) → O(pos×DP). Tiempo: 20min → <1seg"],
    ]
    ...
    for i,row in enumerate(versiones):
        if i == 0:
            ...
            color_tasa = C["verde"] if "99" in row[1] else C["azul2"] if "86" in row[1] else C["negro"]
            rows_v.append([
    ...
    bugs = [
        ...
        ["1","v2","F2 no subdividía grupos grandes correctamente","Pérdida de ~3% de conciliaciones"],
        ["2","v2","F5 se ejecutaba antes de F6g/F7, consumiendo transacciones","F7 no encontraba candidatos"],
        ["3","v2","id_conciliacion negativo causaba JOIN fallido","Grupos F7 no se registraban en DB"],
        ...
        ["6","v3","F7b: doble loop O(neg×pos×DP) → timeout >20min","Proceso bloqueado en producción"],
        ["7","v4","id_lote hardcodeado (1/2) → datos sobreescritos","Historial de meses perdido"],
        ["8","v4","stats-por-fase sin filtro id_lote → datos mezclados","Dashboard mostraba datos incorrectos"],
    ]
    ...
    if historial:
        ...
        rows_h = [[Paragraph(f"<b>{h}</b>", S["normal"]) for h in
                   ["Fecha","Total","Conciliados","Pendientes","Efectividad"]]]
    ...
    estado_data = [
        ...
        [Paragraph("Fases del motor", S["normal"]),      Paragraph("F1 → F2 → F3 → F4 → F6 → F7 → F7b → F5", S["small"])],
        ...
        [Paragraph("Tiempo F7b (v3 vs v4)", S["normal"]),Paragraph(">20 minutos → <1 segundo", S["bold"])],
    ]
...
async def reporte_conciliacion_completa_pdf(id_lote: int = Query(...), cuenta: str = Query(...)):
    ...
    t_kpi.setStyle(TableStyle([
        ("BACKGROUND",(0,0),(0,-1),C["gris_c"]),("BACKGROUND",(1,0),(1,-1),C["verde_c"]),
        ("BACKGROUND",(2,0),(2,-1),C["rojo_c"]), ("BACKGROUND",(3,0),(3,-1),C["azul_c"]),
    ...
    elems.append(HRFlowable(width=page_w, thickness=1.5, color=C["azul2"], spaceAfter=8))
    FASE_DESC = {
        "F1":"Fast-Pass — 1:1 exacto","F2":"Subset Sum — N:N suma cero",
        "F3":"Tolerancia ±5 cts","F4":"Localidad — monto+localidad",
        "F5":"Monto Puro Global","F6":"Subset Sum Global (N pos → 1 neg)",
        "F7":"Final Cleaning (1 pos → N neg)","F7b":"Final Cleaning B",
    }
    cur.execute("""
        SELECT CASE WHEN c.fase_origen='F6g' THEN 'F6' ELSE c.fase_origen END AS fase,
        ...
        GROUP BY fase ORDER BY fase
    """, (id_lote, cuenta))
    fase_rows = [[Paragraph(f"<b>{h}</b>", S["normal"]) for h in ["Fase","Descripción","Grupos","Transacciones","% Conciliados"]]]
    ...
    t_fases = _tabla_std(fase_rows, [page_w*0.07,page_w*0.48,page_w*0.12,page_w*0.15,page_w*0.18], C["azul"], C["azul_c"], C)
...
async def reporte_metricas_fases_pdf(id_lote: int = Query(...), cuenta: str = Query(...)):
    ...
    cur.execute("""
        ...
        GROUP BY fase ORDER BY fase
    """, (id_lote, cuenta))
    hdrs = ["Fase","Grupos","Transacciones","Negativos","Positivos","Monto (COP)","% Total","% Conciliados"]
...
async def reporte_pendientes_pdf(id_lote: int = Query(...), cuenta: str = Query(...)):
    ...
    res_rows = [
        ...
        [Paragraph("Positivos pendientes", S["normal"]),      Paragraph(f"{pos_p or 0:,}", S["verde_v"])],
        [Paragraph("Monto negativos (COP)", S["normal"]),     Paragraph(f"${monto_neg or 0:,.2f}", S["rojo_v"])],
        [Paragraph("Monto positivos (COP)", S["normal"]),     Paragraph(f"${monto_pos or 0:,.2f}", S["verde_v"])],
        [Paragraph("Diferencia neta (COP)", S["normal"]),     Paragraph(f"${(monto_pos or 0)-(monto_neg or 0):,.2f}", S["bold"])],
    ]
    ...
    neg_rows = cur.fetchall()
    if neg_rows:
        elems.append(Paragraph(f"Top {len(neg_rows)} Negativos Pendientes (mayor monto)", S["seccion"]))
        ...
        col_w = [page_w*0.08,page_w*0.07,page_w*0.10,page_w*0.45,page_w*0.12,page_w*0.18]
